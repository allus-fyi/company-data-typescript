...
/**
 * A short-timeout {@link HttpTransport} for the poll clients (contract §"short-cycled SDK waits").
 *
 * The SDK's `pollResult` / `waitForResult` bound their LOGICAL loop with `timeout=2`, but that does not
 * bound the underlying HTTP request. A single-worker server must not be pinned by one blackholed poll,
 * so the poll clients get this transport, which aborts each request via `AbortSignal.timeout`. It mirrors
 * the SDK's default `FetchTransport` otherwise (an injected transport is the SDK's intended seam).
 */
