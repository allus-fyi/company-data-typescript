...
/**
 * One-command launcher for the company-data example (`npm start`).
 *
 * Steps:
 ...
 *   5. serve bundle + API on ONE port — SINGLE process = single worker (also the public POST /webhook)
 */

const RELEASE_BASE = 'https://github.com/allme-sdk/example-test-suite/releases/download';
const base = dirname(dirname(fileURLToPath(import.meta.url))); // examples/company-data
process.chdir(base);

async function main(): Promise<void> {
  log('company-data example — starting up');
